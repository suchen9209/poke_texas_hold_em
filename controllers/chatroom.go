...
var (
	...
	gameprocess_map          map[int]chan models.CardInfo
	userInfoChannel_map      map[int]chan models.Event
	roundprocess_map         map[int]chan models.RoundInfo
	userOperationProcess_map map[int]chan models.UserOperationMsg
	gameop_map               map[int]chan string
	seat_map                 map[int]*list.List
	positionTurn_map         map[int]int //记录当前轮到的玩家位置
	LimitPoint_map           map[int]int //当前轮最小点数值

	nowGameMatch_map map[int]models.GameMatch

	roundUserDetail_map map[int]map[int]models.InRoundUserDetail

	emptySend_map map[int]int //游戏结束时清零 仅用于记录大小盲
...
func chatroom_new(room_id int) {
	for {
		...
		case op := <-gameop:
			if op == "start" {
				if (nowGameMatch_map[room_id].Id == 0 || nowGameMatch_map[room_id].GameStatus == models.GAME_STATUS_END) && seat_map[room_id].Len() >= 2 {
					startGame()
				}
			}
			if op == "show_card3" {
